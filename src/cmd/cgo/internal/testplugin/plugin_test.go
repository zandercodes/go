...
// onWindows is true when the host runs the windows/amd64 plugin path,
// which links plugins against a pre-built host EXE via -pluginhost.
var onWindows = runtime.GOOS == "windows"

// pluginHostFlags returns extra -ldflags so a plugin built on Windows
// is linked against host. The arg is the (relative) path to the host
// EXE; we resolve it to absolute since the linker may run in a
// different cwd.
func pluginHostFlags(host string) []string {
	if !onWindows {
		return nil
	}
	...
	return []string{"-ldflags=-pluginhost=" + abs}
}
...
func testMain(m *testing.M) int {
	...
	// The windows/amd64 plugin port does not require cgo.
	if !onWindows && !testenv.HasCGO() {
	...
	buildPlugin := func(args ...string) {
		a := append([]string{"-buildmode=plugin"}, pluginHostFlags("host.exe")...)
		a = append(a, args...)
		goCmd(nil, "build", a...)
	}
	...
	// Skipped on windows/amd64 because the upstream source uses cgo,
	...
	// On windows/amd64 only a subset of upstream plugin tests is wired
	...
	if onWindows {
		...
		globalSkip = func(t *testing.T) {
			if !subset[t.Name()] {
				t.Skipf("plugin: %s not in windows/amd64 subset", t.Name())
			}
		}
	}
...
func TestUniqueTypesAndItabs(t *testing.T) {
	...
	if onWindows {
		...
		// windows/amd64 -buildmode=plugin: the compiler emits
		...
		t.Skip("cross-plugin type unification not supported on windows; see WinPlugin/README.md")
	}
...
func TestIssue18676(t *testing.T) {
	...
	if onWindows {
		...
		ldflags := pluginHostFlags("issue18676.exe")
		args := append([]string{"-buildmode=plugin"}, ldflags...)
		goCmd(t, "build", append(args, "-o", "plugin.so", "./issue18676/plugin.go")...)
	} else {
