...
func asmbPe(ctxt *Link) {
	...
	// Host-only: embed the plugin host symbol table as a `.gosymt`
	// section so plugin DLLs linked with -pluginhost=<this.exe> can
	// recover it directly from the EXE without a side-car file.
	pefile.addPluginGoSymtab(ctxt)
